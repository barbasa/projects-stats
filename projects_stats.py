...
def _extract_proj_and_kind(line: str):
    ...
    Returns a tuple: (project: Optional[str], kind: Optional[str], discarded_url: Optional[str])
    where kind is one of {"read", "write", None}.
    """
    ...
    if m:
        proj = m.group("proj")
        try:
            return unquote(proj), READ_OPERATION, None
        except Exception:
            return proj, READ_OPERATION, None
    ...
    if m:
        proj = m.group("proj")
        try:
            return unquote(proj), WRITE_OPERATION, None
        except Exception:
            return proj, WRITE_OPERATION, None
    ...
    if path_.startswith('/projects/'):
        ...
        encodedProj = segs[2]
        try:
            return unquote(encodedProj), kind, None
        except Exception:
            logging.warning(f"Cannot decode project {encodedProj}")
            return encodedProj, kind, None
    ...
    if path_.startswith('/changes/'):
        ...
        proj = rest.split('~', 1)[0]
        try:
            return unquote(proj), kind, None
        except Exception:
            return proj, kind, None

    return None, kind, path_
...
def get_last_access_from_logs(logs_dir: str):
    """Scan Gerrit HTTP logs and return (last_reads, last_writes, discarded_urls).
        ...
        try:
            with _open_maybe_gz(path) as fh:
                for line in fh:
                    ...
                    proj, kind, discarded_url = _extract_proj_and_kind(line)
                    if not proj:
                        if discarded_url:
    ...
    return last_reads, last_writes, discarded_urls
...
def main():
    ...
    last_reads_from_logs, last_writes_from_logs, discarded_urls = get_last_access_from_logs(LOGS_PATH)
