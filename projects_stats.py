...
def _extract_proj(line: str):
    ...
    if path_.startswith('/projects/'):
        ...
        try:
            ...
            logging.warning(f"WARN: Cannot decode project {encodedProj}")
...
def get_last_reads_from_logs(logs_dir: str):
    ...
    for path in _iter_log_files(logs_dir):
        try:
            ...
            logging.warning(f"Warning: failed to process {path}: {e}")
    ...
    for repo in repos:
        ...
        if existing_creation:
            ...
            if not creation_date:
                logging.error(f"Error: could not extract creation date for {repo}")
